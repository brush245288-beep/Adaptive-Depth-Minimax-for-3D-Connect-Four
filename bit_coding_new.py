...
def immediate_win_op(p1,p2,player):
    w31 = []
    w32 = []
    ...
    for mask in w_im:
        ...
        if win_position > 15:
            empty_position = win_position - 16
        else:
            return True
        if (board_O & (1 << empty_position)):
            return True
...
def minimax_new_alphabeta(p1,p2, depth, player1, player2, alpha, beta, w_self2, w_self3, w_opp2, w_opp3):
    #key = (encode_normalized_board(p1,p2), depth, player1, player2)
    #if key in minimax_dict:
        #key["visits"] += 1
        #new
        #return minimax_dict[key]
    ...
    if depth == 0:
        return evaluation_bin(p1,p2,player1,player2, w_self2, w_self3, w_opp2, w_opp3)
...
def choose_best_move_dict(p1,p2, player,depth,alpha,beta,w_self2, w_self3, w_opp2, w_opp3):
    ...
    for position in moves:
        ...
        key = (encode_normalized_board(test_p1, test_p2), depth - 1, other_player(player), player)
        if key in minimax_dict:
            score = minimax_dict[key]
        else:
            # One candidate move has already been applied, so search the remaining depth.
