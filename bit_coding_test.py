...
def choose_best_move_dict(p1,p2, player, alpha,beta,w_self2, w_self3, w_opp2, w_opp3, min_depth,smc_timelimit):
    ...
    def time_control():
        ...
        time_b = (dt - smc_time) / smc_timelimit

        print("cuurent depth: ", depth, " dt: ", dt, " time_b: ", time_b)
        if time_b > -0.3:
    ...
    return best_move, record_depth,smc_time

def rein_write():
    '''
    end of N turns
    '''
    keys = minimax_dict_test.keys()
    for key in keys:
        if minimax_dict_test[key]["visits"] > 1:
            rein_dict[key] = minimax_dict_test[key]["score"]
        elif minimax_dict_test[key]["depth"] > 6:
            rein_dict[key] = minimax_dict_test[key]["score"]
