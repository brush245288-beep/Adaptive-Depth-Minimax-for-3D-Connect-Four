...
depth_time = {}
"""
Depth-aware experience-reuse minimax agent.

This file shares the general bitboard and alpha-beta minimax framework
used by the other agent implementations.

For the experience-reuse mechanism, go directly to:

    minimax_new_alphabeta()     ~ line 632

The experience-specific logic includes:
- symmetry-normalized state keys;
- storage of search-derived values and remaining search depth;
- direct reuse when a deeper stored result is available;
- visit-count-weighted smoothing at equal depth;
- replacement when a deeper search result becomes available.
...
def minimax_new_alphabeta_odd(p1, p2, depth, player1, player2, alpha, beta, w_self2, w_self3, w_opp2, w_opp3):
    ...
    if player1 == player2:
        ...
        for position in moves:
            test_p1, test_p2 = play_position(p1, p2, player1, position)
            score = minimax_new_alphabeta(test_p1, test_p2, depth - 1, other_player(player1), player2, alpha, beta,
                                          w_self2, w_self3, w_opp2, w_opp3)
...
# ============================================================
# DEPTH-AWARE EXPERIENCE-REUSE IMPLEMENTATION
# ============================================================
# The board representation, heuristic evaluation and alpha-beta
# search follow the common minimax framework.
#
# Experience-specific additions begin here:
# 1. symmetry-normalized state key;
# 2. reuse of values obtained at greater remaining depth;
# 3. visit-count-weighted smoothing at equal depth;
# 4. replacement when a deeper search becomes available.

def minimax_new_alphabeta(p1,p2, depth, player1, player2, alpha, beta, w_self2, w_self3, w_opp2, w_opp3):
...
def choose_best_move_dict(p1,p2, player, alpha,beta,w_self2, w_self3, w_opp2, w_opp3):
    depth = 3
    ...
    for position in moves:
        ...
        #score = minimax_new_alphabeta_odd(test_p1, test_p2, depth-1, other_player(player), player, alpha, beta, w_self2, w_self3, w_opp2, w_opp3)
        score = minimax_new_alphabeta(test_p1, test_p2, depth-1, other_player(player), player, alpha, beta, w_self2, w_self3, w_opp2, w_opp3)
        if score > best_score:
            best_score = score
            best_move = position
