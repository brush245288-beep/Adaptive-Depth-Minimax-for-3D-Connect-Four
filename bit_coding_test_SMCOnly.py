...
"""
SMC-inspired adaptive-depth minimax agent.

This file shares the board representation, heuristic evaluation,
alpha-beta search, symmetry reduction, and move ordering used by the
Improved minimax implementation.

For the SMC-specific implementation, the recommended reading order is:

1. choose_best_move_dict()      ~ line 700
   - iterative deepening
   - within-decision continuation rule
   - integration with the time controller

2. smc_update_time_limit()      ~ line 870
   - SMC-inspired between-decision time-budget update

Other sections largely contain the shared minimax implementation.

Note:
The experimental half_gear() and MCTS implementations were not used
in the dissertation experiments.
"""
...
def evaluation_bin(p1, p2, player1,player, w_self2, w_self3, w_opp2, w_opp3):
    potential3_p1 = []
    potential3_p2 = []
    potential2_p1 = []
    potential2_p2 = []
    ...
    score_cm2 = 0
    #====p1====
    for i in range(len(mask_win_p1) - 1):
        for j in range(i + 1, len(mask_win_p1)):
            c_m = (mask_win_p1[i] & mask_win_p1[j]).bit_count()
            if c_m == 1:
                score_cm1 += wcm_p1
    # ====p2====
    for i in range(len(mask_win_p2) - 1):
        for j in range(i + 1, len(mask_win_p2)):
...
def minimax_new_alphabeta(p1,p2, depth, player1, player2, alpha, beta, w_self2, w_self3, w_opp2, w_opp3,node_count):
    ...
    if depth == 0:
        node_count += 1
        return evaluation_bin(p1, p2, player1,player2, w_self2, w_self3, w_opp2, w_opp3),node_count

    moves = get_valid_moves_bit(p1, p2)
    moves.sort(key=lambda position: move_order_score(p1, p2, player1, position), reverse=True)

    if not moves:
        score = evaluation_bin(p1, p2, player1,player2, w_self2, w_self3, w_opp2, w_opp3)
        node_count += 1
        # minimax_dict[key] = score
        return score,node_count
    ...
    if player1 == player2:
        ...
        for position in moves:
            test_p1, test_p2 = play_position(p1, p2, player1, position)
            score, node_count = minimax_new_alphabeta(test_p1,test_p2,depth - 1,other_player(player1),player2,alpha,beta,w_self2,w_self3,w_opp2,w_opp3,node_count)
            best_score = max(best_score, score)
            alpha = max(alpha, best_score)
...
# ============================================================
# SMC-SPECIFIC ADAPTIVE-DEPTH IMPLEMENTATION
# ============================================================
# The board representation, heuristic evaluation and alpha-beta
# search above follow the Improved minimax implementation.
#
# The dissertation-specific SMC additions begin here:
# 1. iterative deepening with depth increments of two;
# 2. within-decision continuation test;
# 3. between-decision time-budget update.

def choose_best_move_dict(p1,p2, player, alpha,beta,w_self2, w_self3, w_opp2, w_opp3, min_depth,smc_timelimit):
    depths = 22
    best_list = []
    board_check = p1 | p2
    ...
    def time_control():
        ...
        smc_time = smc_update_time_limit(smc_timelimit, dt)

        time_b = (dt-smc_timelimit)/smc_timelimit

        print("current depth: ", depth, " dt: ", dt," time_b: ",time_b)

        if time_b > -0.3:
            return False, smc_time
        return True, 0.1 # always reach to max depth (game solved),and return the target time for next game


    def half_gear(depth): #further work, not include in the current research
        """
           Experimental depth-extension idea developed during the project.
           Not used in the dissertation experiments.
        """
        best_list.sort(key=lambda position: position[1], reverse=True)
        best_move = None
        top_3 = best_list[:2]
        depth += 1
        for position_first, best_score_first in top_3:
            half_gear_position1, half_gear_position2 = play_position(p1, p2, player, position_first)
            moves = get_valid_moves_bit(half_gear_position1, half_gear_position2)
            moves.sort(key=lambda position: move_order_score(half_gear_position1, half_gear_position2, other_player(player), position), reverse=True)
            extended_best_score = float("inf")

            for position in moves:
                test_gear_p1, test_gear_p2 = play_position(half_gear_position1, half_gear_position2, other_player(player), position)
                extended_score = minimax_new_alphabeta(test_gear_p1, test_gear_p2, depth, player, player, alpha, beta, w_self2, w_self3, w_opp2, w_opp3, start_time)
            if extended_score > half_best_score:
                half_best_score = extended_score
                half_best_move = position_first

        return best_move,half_best_move
    ...
    node_count = 0
    node_flag = False
    for depth in range(min_depth,depths,2):
        best_score = -float("inf")
        best_move = None
        for position in moves:
            ...
            score,node_count = minimax_new_alphabeta(test_p1, test_p2, depth - 1, other_player(player), player, alpha, beta, w_self2, w_self3, w_opp2, w_opp3,node_count)
            ...
            if node_count >= 5000:
                ...
                break
            '''
        smc_bool, smc_time = time_control()
        best_list.append([best_move,node_count])
        if not smc_bool:
            if node_flag == True:
                record_depth = depth - 2
                best_move = best_list[-2][0]
                node_count = best_list[-2][1]
                break
            record_depth = depth
            break
    ...
    return best_move, record_depth,smc_time,node_count
...
def move_order_score(p1, p2, player, position):
    ...
    score_co = corner(position) * 2
    total_score = score_ce + score_p + score + score_co
    noice = random.uniform(-0.02, 0.02) * total_score
    return total_score + noice

#============SMC-inspired time-budget controller=============

def sat(x: float) -> float:
    return max(-1.0, min(1.0, x))

def smc_update_time_limit(time_limit, actual_time, target_time=0.1, phi=0.25, k=0.1, min_time=0.05, max_time=0.5):
    s = actual_time - target_time

    u = sat(s / phi)

    time_limit *= (1 - k * u)

    return max(min_time, min(time_limit, max_time))
