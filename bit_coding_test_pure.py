...
minimax_dict_test = None

def get_minimax_dict(path="minimax_dict.pkl"):
    global minimax_dict_test
    if minimax_dict_test is None:
        minimax_dict_test = load_mdict_table(path=path)
    return minimax_dict_test
...
def minimax_new_alphabeta(p1,p2, depth, player1, player2, alpha, beta, w_self2, w_self3, w_opp2, w_opp3,start_time):
    ...
    moves.sort(key=lambda position: move_order_score(p1, p2, player1, position, moves), reverse=True)
...
def choose_best_move_dict(p1,p2, player, alpha,beta,w_self2, w_self3, w_opp2, w_opp3):
    ...
    moves.sort(key=lambda position: move_order_score(p1, p2, player, position, moves), reverse=True)

    best_move = None
    best_score = -float("inf")
    best_source = "heuristic"
    best_dict_depth = None
    hits = 0
    ...
    for position in moves:
        ...
        key = (encode_normalized_board(test_p1, test_p2), other_player(player), player)
        entry = table.get(key)

        if entry is not None:
            score = entry_score(entry)
            source = "dict"
            dict_depth = entry_depth(entry)
            hits += 1
        else:
            score = evaluation_bin(test_p1, test_p2, player, w_self2, w_self3, w_opp2, w_opp3)
            source = "heuristic"
            dict_depth = None
...
def move_order_score(p1, p2, player, position,moves):
...
def smc_update_time_limit(time_limit, actual_time, target_time=1.0, phi=0.5, k=0.25, min_time=0.5, max_time=2.0):
