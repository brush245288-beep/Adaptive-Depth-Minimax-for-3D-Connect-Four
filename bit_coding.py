...
"""
Core Improved minimax implementation for 4x4x4 Connect Four.

Recommended starting point for reading the dissertation code.
...
The SMC and experience-reuse agents are implemented in separate files
and reuse substantially the same underlying search framework.
"""
...
def three_types_sticks():
    """
       Generate vertical (z-axis) line masks used for floating-threat checks.

       The function name is retained from an earlier implementation that
       considered x-, y-, and z-direction line masks.
    """
    z_blades = []
...
z_blade = three_types_sticks()
diag_a = diag_assembly()
...
#-------------Bitboard encoding and symmetry reduction---------------
...
def encode_normalized_board(player1_board,player2_board):
    # Generate symmetry-equivalent bitboards and select the
    # lexicographically smallest pair as the canonical state.
...
def evaluation_bin(p1, p2, player1,player, w_self2, w_self3, w_opp2, w_opp3):
    ...
    # Evaluate from the root player's perspective.
    # The player-to-move changes at each ply and is used for turn-dependent threats.
    if player == 1:
        w_12 = w_self2
        w_13 = w_self3
        w_f1 = 0.17 * w_self3
    ...
    #======Intersecting-threat evaluation======
...
def choose_best_move_dict(p1,p2, player,depth,alpha,beta,w_self2, w_self3, w_opp2, w_opp3):
    best_score = -float("inf")
    best_move = None
    node_count = 0
    moves = get_valid_moves_bit(p1, p2)
...
def potential_connect2(p1,p2, player):
    '''
        Score for Open two-piece line, use in rearrange the move order list
    '''
    if player == 1:
        player_bits = p1
        opponent_bits = p2
...
def potential_connect3(p1,p2, player):
    '''
    Score for Open three-piece line, use in rearrange the move order list
    '''
    if player == 1:
        player_bits = p1
        opponent_bits = p2
...
def move_order_score(p1, p2, player, position):
    ...
    score_p2 = potential_connect2(test_p1,test_p2, player) * 4
    #score_p3 = potential_connect3(test_p1,test_p2, player) * 2
    score_co = corner(position) * 2
    total_score = score_ce + score_p2 + score + score_co
    noice = random.uniform(-0.02, 0.02) * total_score # Small random perturbation for tie-breaking in move ordering
    return total_score + noice

#------------------------MCTS------------------------------
# Experimental MCTS implementation
# Not used in the experiments reported in the dissertation.
